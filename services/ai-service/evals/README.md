...
## LangGraph mental model (written from memory)

**How does a `StateGraph` differ from writing a `while tool_calls: ...` loop? What does the graph give you that the loop doesn't?**
A hand-rolled loop is one function: the reasoning step, the dispatch step, and the error handling are all inline in the same block, so the only way to test "what happens when the LLM asks for a tool" is to actually run the loop with a real or mocked LLM. A `StateGraph` pulls each of those responsibilities apart into its own node — `agent` (call the LLM), `tools` (dispatch whatever it asked for), `fallback` (recover from a failure) — connected by an explicit routing function (`should_continue`). Nothing about the *behavior* changes; the same Reason → Act → Observe → Reason cycle still runs. What changes is that the routing decision is now a plain function over a typed state dict, callable and assertable with zero LLM or network calls — that's exactly what `test_advisor_agent.py` does: it feeds `should_continue` a state with `tool_calls` set and asserts it returns `"tools"`, without ever touching Claude's API. A `while` loop has no such seam; the routing logic is welded to the call site. The graph also gives the loop a shape you can *see* — `build_graph()` is a literal topology (nodes + edges) that Langfuse can render as a trace with named hops, instead of an opaque stack of function calls.
...
`thread_id` is just a string key `MemorySaver` uses to look up saved state — it carries no meaning to LangGraph itself, the way a dictionary key carries no meaning to a `dict`. After `advisor_graph.ainvoke(initial_state, config={"configurable": {"thread_id": session_id}})` finishes, the checkpointer writes the final `AdvisorState` (messages, fetched data, everything) to its store under that key. The *next* call with the same `thread_id` doesn't start from `initial_state` at all — LangGraph loads the saved state first and merges the new input on top of it. Two different UUIDs are two completely independent conversations even inside the same process, which is why `AdvisorService.ask()` generates a fresh `uuid4()` whenever the request omits `session_id`: no shared key, no shared memory, by construction rather than by convention.
...
LangGraph's default behavior for merging a node's return value into state is: replace whatever's in that field with whatever the node returned. That's fine for a scalar like `error` — a node that returns `{"error": None}` genuinely means "the error is now None." It's wrong for `messages`: `call_agent` only ever returns `{"messages": [response]}` — the *one new message* it produced, not the whole history — so a plain `list[BaseMessage]` field would overwrite turn 1's entire conversation with a single-element list every time the agent node runs. `add_messages` is a *reducer* — a merge function LangGraph calls instead of the default overwrite, and for this field it appends the new message(s) onto the existing list rather than replacing it. Without it, the graph doesn't error or warn; it just quietly forgets everything except the most recent message, which is the exact silent-failure shape described in the chapter's reference-resolution bug (a follow-up like "bulan itu" has nothing to resolve against because the turn that established the referent is gone).
