...
| Step | Observe (input to LLM) | Reason (LLM's decision) | Act (tool call) | Result fed back | Data source (where it actually looks) |
|------|------------------------|--------------------------|------------------|------------------|-----------------------------------------|
| 1 | Task text only — nothing gathered yet | System prompt says rules first → try keyword `grab` | `search_category_rules(keyword="grab")` | `"No rules matched."` — the word "GRAB" alone isn't a rule pattern; `"gojek"`/`"grabcar"` might be but `"grab"` didn't hit | **In-memory snapshot**, not a live DB call. `_CATEGORY_RULES` dict ([category_rules.py:11](../../../services/ai-service/app/agents/tools/category_rules.py#L11)) is populated once from `SELECT keyword, category FROM category_rules` (Postgres) during service startup ([main.py:79-129](../../../services/ai-service/app/main.py#L79-L129)) and never re-queried until restart. |
| 2 | Rules came up empty → not enough evidence to answer yet | Prompt says: on empty/ambiguous rules, try similarity next | `find_similar_transactions(description="GJ*GRAB CAR JAKARTA")` | `"Similar past transactions:\n  [1] 'GJ*GRAB CAR SBY' — Transportation (Online) (similarity=0.94)\n  [2] 'GOJEK RIDE' — Transportation (Online) (similarity=0.81)\n  [3] 'GJ*GRABFOOD' — Food & Dining (similarity=0.62)"` | **Live query**, not cached. Hits `app.state.retriever` ([similarity.py](../../../services/ai-service/app/agents/tools/similarity.py)) — the same pgvector-backed retriever the `/search` endpoint uses — so newly-ingested transactions are searchable immediately, unlike steps 1 and 3. |
| 3 | 2 of 3 similar transactions agree on `Transportation (Online)`; the third is a different Grab product (food, not ride) | Has enough evidence now — needs the exact valid category string before answering | `list_all_categories()` | `"Valid categories:\n  - Transportation (Online)\n  - Food & Dining\n  ..."` — confirms `Transportation (Online)` is a real category name, not a guess | **In-memory snapshot**, same pattern as step 1. `_CATEGORIES` list ([categories.py:10](../../../services/ai-service/app/agents/tools/categories.py#L10)) is loaded once at startup from the same DB read that feeds the query planner's vocabulary ([main.py:115](../../../services/ai-service/app/main.py#L115)); falls back to a hardcoded list only if the DB was unreachable at boot. |
| — (final, no more tool calls — `max_steps=3` reached) | All 3 tool outputs in context | Synthesizes an answer *without* a 4th tool call, since the step budget is spent | — | LLM emits free text | — no tool call, no data source; answer is synthesized purely from what's already in context |
